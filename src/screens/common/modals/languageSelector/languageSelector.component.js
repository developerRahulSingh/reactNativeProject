...
import { componentStyle } from './languageSelector.style';
...
export default class LanguageSelector extends BaseModal {
  constructor(props) {
    super(props);
  }
      ...
      <View style={[componentStyle.backdrop]}>
        <View style={[componentStyle.container]}>
              ...
              {this.props.items.map((item, index: number) => {
                ...
                return (
                  <TouchableOpacity activeOpacity={0.6} key={index} style={componentStyle.labelContainer}
                    ...
                    <RegularText style={[itemStyle, componentStyle.labelTextStyle]}>{strings(`languageSelectorComponent.${item.label}`)}</RegularText>
                  </TouchableOpacity>
                );
              })}
            </View>
          ...
          <StandardButton showCompact={true} isBottomButton={true} color={commonTheme.COLOR_PRIMARY_DARK}
                          labelText={strings('languageSelectorComponent.label_button_cancel')} onPress={this.close}/>
        </View>
      </View>
    );
  }
...
LanguageSelector.propTypes = _propTypes;
LanguageSelector.defaultProps = _defaultProps;
