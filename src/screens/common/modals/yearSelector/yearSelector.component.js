...
import { componentStyle } from './yearSelector.style';
...
export default class YearSelector extends BaseModal {
  constructor(props) {
    super(props);
  }
      ...
      <View style={[componentStyle.backdrop]}>
        <View style={[componentStyle.container]}>
          <ScrollView bounces={false} contentContainerStyle={[componentStyle.listView]}>
            <View style={componentStyle.subContainer}>
              {this.props.items.map((item, index: number) => {
                ...
                return (
                  <TouchableOpacity activeOpacity={0.6} key={index} style={componentStyle.itemContainer}
                                    onPress={async () => this._onItemSelected(item)}>
                    <RegularText style={[itemStyle, componentStyle.itemTextStyle]}>{item.key}</RegularText>
                  </TouchableOpacity>
                );
              })}
            </View>
          ...
          <StandardButton showCompact={true} isBottomButton={true} color={commonTheme.COLOR_PRIMARY_DARK}
                          labelText={strings('yearSelectorComponent.label_button_cancel')} onPress={this.close}/>
        </View>
      </View>
    );
  }
...
YearSelector.propTypes = _propTypes;
YearSelector.defaultProps = _defaultProps;
